...
필터는 각 서버 안의 작업이 아니라 **서버 목록**에 적용합니다.

| 필터 | 표시하는 서버 |
| --- | --- |
| 전체 | 등록된 모든 서버. 조회를 중지한 서버도 포함 |
| 실행 | SSH 조회가 켜져 있고, 최신 정보에서 활성 GPU가 확인된 서버 |
| 조회 | SSH 조회가 켜진 모든 서버. GPU 유휴 상태와 연결 재시도 중인 서버도 포함 |

‘실행’은 메뉴바 GPU 아이콘과 같은 기준을 사용합니다. GPU 사용률, 연산 프로세스 또는 GPU에 연결된 실행 pane이 확인되어야 하며, 메모리 할당이나 CPU 작업만으로는 실행으로 표시하지 않습니다. 연결이 끊기거나 관측값이 오래되면 실행에서 제외합니다. 전체 자동 조회를 일시 정지하면 실행·조회 목록은 비워지고 다시 시작하는 안내를 표시합니다.

표시된 서버 안에서는 모든 tmux 작업을 볼 수 있습니다. 작업별 **감시**는 알림 설정으로 유지되며 서버 필터와 독립적입니다. GPU 작업이 끝나 서버가 실행 목록에서 빠져도 이미 연 로그창은 유지합니다.
...
Sources/GPUMonitor/
  ...
  Presentation.swift        서버 필터·작업 정렬·표시 도우미
