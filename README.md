...
앱의 **서버 추가 → 별칭 입력 옆 목록 버튼**에서 `training-server`를 선택하면 됩니다. 신규 설치에는 기본 서버가 등록되어 있지 않습니다. 기존 사용자의 저장된 설정은 유지됩니다.
...
대시보드·설정·서버 추가·로그 창에 같은 여백, 서체, 카드와 조작 규칙을 적용했습니다. 떠 있는 버튼과 선택 컨트롤은 Liquid Glass, 수치와 로그는 읽기 쉬운 표준 머티리얼을 사용합니다. 카드 안의 작은 조작은 유리를 중첩하지 않고 선택·호버 상태로 구분합니다.
...
**단축키:** `⌘R` 새로고침, `⌘N` 서버 추가(대시보드), `⌘,` 설정, `Esc` 보조창 닫기, `⌘Q` 앱 종료. 입력창의 편집과 로그의 선택·복사에 macOS 기본 단축키를 사용할 수 있습니다.
...
Sources/GPUMonitor/
  ...
  ServerViews.swift         서버·GPU·세션·작업 카드
