...
@main struct PresentationChecks {
    ...
    static func main() {
        ...
        var preferences = Preferences(selected: "a/%2", watched: ["b/%3", "a/%1"])
        require(Set(state.visibleJobs(serverID: "a", preferences: preferences).map { $0.pane.id }) == ["%0", "%1", "%2"], "Server filters must retain running and idle jobs plus tracked missing panes, without importing another server's watches")
        preferences.watched.insert("a/%3")
        require(state.visibleJobs(serverID: "a", preferences: preferences).contains { $0.pane.id == "%3" }, "A vanished watched pane must remain visible")
        print("PASS server contents retain idle jobs and tracked missing panes with server-scoped watches")
        ...
        require(sorted.visibleJobs(serverID: "a", preferences: Preferences()).map { $0.pane.id } == ["b", "a", "c", "d"], "Running jobs first, natural session order, numeric pane order")
